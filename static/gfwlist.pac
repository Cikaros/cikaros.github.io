...
var proxy = "SOCKS5 proxy.netbird.local:7890; SOCKS5 rainyun.netbird.local:7890; PROXY rainyun.netbird.local:7890; DIRECT";
...
var isContain = function (start, end, ip) {
    ...
    if (star_d.length != ip_d.length || end_d.length != ip_d.length) {
        return false
    }
    for (var i = 0; i < 4; i++) {
        var ip_item = ip_d[i];
